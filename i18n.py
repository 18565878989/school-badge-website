# Flask Multilingual Support

from flask import Flask, request, session, g

# Language translations
TRANSLATIONS = {
    'zh': {
        # Navigation
        'home': '首页',
        'campus_gallery': '校园风光',
        'my_likes': '我的收藏',
        'login': '登录',
        'register': '注册',
        'logout': '退出',
        'language': '语言',
        'admin_panel': '管理后台',
        
        # Home
        'hero_title': '探索全球名校',
        'page_title_home': '首页',
        'hero_subtitle': 'Discover prestigious institutions worldwide',
        'search_placeholder': '搜索学校名称、城市或国家...',
        'all_regions': '所有地区',
        'all_countries': '所有国家',
        'all_cities': '所有城市',
        'all_districts': '所有区域',
        'all_types': '所有类型',
        'university': '大学',
        'middle_school': '中学',
        'elementary_school': '小学',
        'kindergarten': '幼儿园',
        'clear_filters': '清除筛选',
        'schools_count': '共 {} 所学校',
        'no_schools': '没有找到符合条件的学校',
        'view_all': '查看全部学校',
        
        # School Card
        'founded': '建校年份',
        'principal': '现任校长',
        'address': '详细地址',
        'location': '位置',
        'region': '地区',
        
        # School Detail
        'back': '返回学校列表',
        'like': '收藏',
        'liked': '已收藏',
        'download': '下载校徽',
        'likes_count': '{} 人收藏',
        'motto_label': '校训',
        'website': '官方网站',
        'visit_website': '访问官网',
        'visit_website_full': '访问 {} 官网',
        'view_on_maps': '在 Google Maps 中查看',
        'map_location': '地图位置',
        'description_title': '学校简介',
        
        # Auth
        'username': '用户名',
        'password': '密码',
        'email': '邮箱',
        'all_required': '所有字段都为必填项。',
        'register_success': '注册成功！请登录。',
        'username_exists': '用户名或邮箱已被注册。',
        'login_error': '用户名或密码错误。',
        'welcome_back': '欢迎回来，{}！',
        'logout_success': '您已退出登录。',
        'please_login': '请先登录后再访问此页面。',
        'already_account': '已有账号？',
        'no_account': '还没有账号？',
        
        # Admin
        'admin': '管理员',
        'dashboard': '仪表盘',
        'manage_schools': '学校管理',
        'manage_users': '用户管理',
        'settings': '系统设置',
        'add_school': '添加学校',
        'edit_school': '编辑学校',
        'delete_school': '删除学校',
        'school_name': '学校名称',
        'school_name_cn': '中文名称',
        'region': '地区',
        'district': '区域',
        'supervisor': '校监',
        'phone': '电话',
        'fax': '传真',
        'gender_type': '性别类型',
        'gender_coed': '男女校',
        'gender_girls': '女校',
        'gender_boys': '男校',
        'finance_type': '办学类型',
        'finance_government': '官立',
        'finance_aided': '资助',
        'finance_direct': '直资',
        'finance_private': '私立',
        'session_type': '授课时间',
        'session_wd': '全日',
        'session_am': '上午',
        'session_pm': '下午',
        'school_code': '学校编号',
        'school_type': '学校类别',
        'country': '国家',
        'city': '城市',
        'description': '简介',
        'badge': '校徽',
        'motto': '校训',
        'founded_year': '建校年份',
        'principal_name': '校长姓名',
        'save': '保存',
        'cancel': '取消',
        'delete': '删除',
        'confirm_delete': '确认删除？',
        'success': '操作成功！',
        'error': '操作失败！',
        'no_permission': '您没有权限访问此页面。',
        'welcome_admin': '管理员，您好！',
        'total_schools': '学校总数',
        'total_users': '用户总数',
        'total_visits': '访问量',
        
        # Footer
        'copyright': '© 2024 校徽网 · School Badge Collection',
        
        # Rankings
        'rankings': '排行榜',
        'rankings_title': '世界大学排行榜',
        'world_university_rankings': '🏆 世界大学排行榜',
        'rankings_subtitle': '汇集QS、U.S. News、THE、ARWU、CWUR五大权威排名',
        'rankings_stats': '共 {} 所上榜学校 · 5 大排名机构',
        'all_rankings': '综合排名',
        'ranking_systems': '排名系统',
        'schools_with_rankings': '上榜学校',
        'top_200_schools': 'Top 200 学校',
        'view_details': '查看详情',
        'no_rankings_yet': '排名数据加载中',
        'rankings_coming_soon': '正在整理五大排名的顶尖学校数据...',
        'social': '社交',
        'page_title_social': '社交',
        'ranking_color_legend': '排名颜色说明',
        'top_10': 'Top 10',
        'top_50': 'Top 50',
        'top_100': 'Top 100',
        'top_200': 'Top 200',
        'page_title_deep_search': 'AI 深度搜索 - 校徽网',
        'deep_search': 'AI 深度搜索',
        'deep_search_desc': '用自然语言描述您想要的学校，AI帮您智能匹配',
        'ai_helper_greeting': '您好！我是校徽网AI助手 🎓',
        'try_examples': '请用自然语言描述您想找的学校，例如：',
        'search_placeholder_deep': '描述您想找的学校...',
        'enter_send_hint': '按 Enter 发送，Shift + Enter 换行',
        'share_title': '分享',
        'share_to_social': '分享到社交平台',
        'copy_link': '复制链接',
        'copied_success': '已复制!',
        'loading': '加载中...',
        'close': '关闭',
        'membership_center': '会员中心',
        'current_membership': '当前会员',
        'bronze_member': '青铜会员',
        'silver_member': '白银会员',
        'gold_member': '黄金会员',
        'my_orders': '我的订单',
        'no_orders_yet': '暂无订单',
        'shopping_cart': '购物车',
        'cart_empty': '购物车是空的',
        'add_to_cart': '添加到购物车',
        'checkout_now': '立即结算',
        'total_amount': '合计',
        'badge_evolution': '校徽演进历史',
        'china_famous': '中国名校',
        'north_america_famous': '北美名校',
        'asia_famous': '亚洲名校',
        'more_schools': '更多名校',
        'design_concept': '设计理念',
        'designer': '设计师',
        'designer_unknown': '不详',
        'change_description': '变更说明',
        'major_events': '重大事件',
        'founding': '创立',
        'renaming': '更名',
        'merger': '合并',
        'expansion': '扩建',
        'award': '荣誉',
        'crisis': '危机',
        'reform': '改革',
        'milestone': '里程碑',
        'digital_collection': '数字藏品',
        'buy': '购买',
        'physical_goods': '实体商品',
        'since': '至今',
        'campus_explore': '校园探索',
        'location_info': '位置信息',
        'principal_label': '校长',
        'visit_website': '访问',
        'view_more_campus': '查看更多校园风光',
        'back_to_social': '返回社交版块',
        'publish_reply': '发布回复',
        'reply_success': '回复成功！',
        'reply_need_login': '请先登录',
        'reply_too_short': '回复至少5个字符',
        'membership_unlock': '解锁更多特权，享受专属服务',
        'open_vip_privilege': '开通会员享专属特权',
        'valid_until': '有效期至',
        'select_membership_level': '选择会员等级',
        'membership_benefits': '会员专属权益',
        'unlimited_sharing': '无限分享',
        'remove_ads': '去广告',
        'exclusive_badge': '专属徽章',
        'hd_download': '高清下载',
        'priority_activities': '优先活动',
        'exclusive_support': '专属客服',
        'member_orders': '会员订单',
        'shop_orders': '商城订单',
        'recommended': '推荐',
        'page_title_shop': '校友商城 - 校徽网',
        'alumni_shop': '校友商城',
        'home_tab': '首页',
        'my_orders_tab': '我的订单',
        'alumni_discount': '校友专属优惠',
        'shop_products': '精选校徽、文创产品，限时折扣中！',
        'buy_now': '立即选购',
        'all_categories': '全部',
        'share_wonderful_content': '分享校徽网精彩内容',
        'please_wait': '请稍候...',
        'from_schoolbadge': '来自 校徽网',
        'share_to': '分享到',
        'wechat': '微信',
        'weibo': '微博',
        'douyin': '抖音',
        'xiaohongshu': '小红书',
        'generate_qr': '生成二维码分享',
        'scan_qr_share': '扫码分享',
        'scan_qr_desc': '扫码二维码打开链接',
    },
    'en': {
        # Navigation
        'home': 'Home',
        'campus_gallery': 'Campus Gallery',
        'my_likes': 'My Likes',
        'login': 'Login',
        'register': 'Register',
        'logout': 'Logout',
        'language': 'Language',
        'admin_panel': 'Admin Panel',
        
        # Home
        'hero_title': 'Explore Global Universities',
        'page_title_home': 'Home',
        'hero_subtitle': 'Discover prestigious institutions worldwide',
        'search_placeholder': 'Search by school name, city or country...',
        'all_regions': 'All Regions',
        'all_countries': 'All Countries',
        'all_cities': 'All Cities',
        'all_districts': 'All Districts',
        'all_types': 'All Types',
        'university': 'University',
        'middle_school': 'Middle School',
        'elementary_school': 'Elementary School',
        'kindergarten': 'Kindergarten',
        'clear_filters': 'Clear Filters',
        'schools_count': '{} schools found',
        'no_schools': 'No schools found matching your criteria',
        'view_all': 'View All Schools',
        
        # School Card
        'founded': 'Founded',
        'principal': 'President / Principal',
        'address': 'Address',
        'location': 'Location',
        'region': 'Region',
        'district': 'Stadtteil',
        'supervisor': 'Aufsichtsperson',
        'phone': 'Telefon',
        'fax': 'Fax',
        'gender_type': 'Geschlecht',
        'gender_coed': 'Gemischt',
        'gender_girls': 'Mädchen',
        'gender_boys': 'Jungen',
        'finance_type': 'Finanzierungsart',
        'finance_government': 'Staatlich',
        'finance_aided': 'Zuschuss',
        'finance_direct': 'Direkte Hilfe',
        'finance_private': 'Privat',
        'session_type': 'Sitzung',
        'session_wd': 'Ganzer Tag',
        'session_am': 'Vormittag',
        'session_pm': 'Nachmittag',
        'school_code': 'Schulcode',
        'school_type': 'Schultyp',

        # School Detail
        'back': 'Back to All Schools',
        'like': 'Like',
        'liked': 'Liked',
        'download': 'Download Badge',
        'likes_count': '{} likes',
        'motto_label': 'Motto',
        'website': 'Official Website',
        'visit_website': 'Visit Website',
        'visit_website_full': 'Visit {} Website',
        'view_on_maps': 'View on Google Maps',
        'map_location': 'Map Location',
        'description_title': 'About',
        
        # Auth
        'username': 'Username',
        'password': 'Password',
        'email': 'Email',
        'all_required': 'All fields are required.',
        'register_success': 'Registration successful! Please login.',
        'username_exists': 'Username or email already exists.',
        'login_error': 'Invalid username or password.',
        'welcome_back': 'Welcome back, {}!',
        'logout_success': 'You have been logged out.',
        'please_login': 'Please login to access this page.',
        'already_account': 'Already have an account?',
        'no_account': "Don't have an account?",
        
        # Admin
        'admin': 'Admin',
        'dashboard': 'Dashboard',
        'manage_schools': 'Manage Schools',
        'manage_users': 'Manage Users',
        'settings': 'Settings',
        'add_school': 'Add School',
        'edit_school': 'Edit School',
        'delete_school': 'Delete School',
        'school_name': 'School Name',
        'school_name_cn': 'Chinese Name',
        'region': 'Region',
        'country': 'Country',
        'city': 'City',
        'description': 'Description',
        'badge': 'Badge',
        'motto': 'Motto',
        'founded_year': 'Founded Year',
        'principal_name': 'Principal Name',
        'save': 'Save',
        'cancel': 'Cancel',
        'delete': 'Delete',
        'confirm_delete': 'Are you sure?',
        'success': 'Success!',
        'error': 'Error!',
        'no_permission': 'You do not have permission to access this page.',
        'welcome_admin': 'Welcome, Admin!',
        'total_schools': 'Total Schools',
        'total_users': 'Total Users',
        'total_visits': 'Total Visits',
        
        # Footer
        'copyright': '© 2024 School Badge Collection',
        
        # Rankings
        'rankings': 'Rankings',
        'rankings_title': 'World University Rankings',
        'world_university_rankings': '🏆 World University Rankings',
        'rankings_subtitle': 'Comprehensive coverage of QS, U.S. News, THE, ARWU, and CWUR rankings',
        'all_rankings': 'All Rankings',
        'ranking_systems': 'Ranking Systems',
        'schools_with_rankings': 'Ranked Schools',
        'top_200_schools': 'Top 200 Schools',
        'view_details': 'View Details',
        'no_rankings_yet': 'Loading Rankings',
        'rankings_coming_soon': 'Compiling top universities from five major rankings...',
        'social': 'Social',
        'page_title_social': 'Social',
        'ranking_color_legend': 'Ranking Color Guide',
        'top_10': 'Top 10',
        'top_50': 'Top 50',
        'top_100': 'Top 100',
        'top_200': 'Top 200',
        'page_title_deep_search': 'AI Deep Search - School Badge Collection',
        'deep_search': 'AI Deep Search',
        'deep_search_desc': 'Describe the school you want in natural language, AI helps you find it',
        'ai_helper_greeting': 'Hello! I am the School Badge AI Assistant 🎓',
        'try_examples': 'Please describe the school you are looking for, for example:',
        'search_placeholder_deep': 'Describe the school you are looking for...',
        'enter_send_hint': 'Press Enter to send, Shift+Enter for new line',
        'share_title': 'Share',
        'share_to_social': 'Share to Social Media',
        'copy_link': 'Copy Link',
        'copied_success': 'Copied!',
        'loading': 'Loading...',
        'close': 'Close',
        'membership_center': 'Membership Center',
        'current_membership': 'Current Membership',
        'bronze_member': 'Bronze',
        'silver_member': 'Silver',
        'gold_member': 'Gold',
        'my_orders': 'My Orders',
        'no_orders_yet': 'No orders yet',
        'shopping_cart': 'Cart',
        'cart_empty': 'Cart is empty',
        'add_to_cart': 'Add to Cart',
        'checkout_now': 'Checkout',
        'total_amount': 'Total',
        'badge_evolution': 'Badge Evolution History',
        'china_famous': 'China Famous Schools',
        'north_america_famous': 'North America Famous',
        'asia_famous': 'Asia Famous Schools',
        'more_schools': 'More Schools',
        'design_concept': 'Design Concept',
        'designer': 'Designer',
        'designer_unknown': 'Unknown',
        'change_description': 'Change Description',
        'major_events': 'Major Events',
        'founding': 'Founding',
        'renaming': 'Renaming',
        'merger': 'Merger',
        'expansion': 'Expansion',
        'award': 'Award',
        'crisis': 'Crisis',
        'reform': 'Reform',
        'milestone': 'Milestone',
        'digital_collection': 'Digital Collection',
        'buy': 'Buy',
        'physical_goods': 'Physical Goods',
        'since': 'Present',
        'campus_explore': 'Campus Explore',
        'location_info': 'Location Info',
        'principal_label': 'Principal',
        'visit_website': 'Visit',
        'view_more_campus': 'View More Campus',
        'back_to_social': 'Back to Social',
        'publish_reply': 'Post Reply',
        'reply_success': 'Reply posted!',
        'reply_need_login': 'Please login first',
        'reply_too_short': 'Reply must be at least 5 characters',
    },
    'fr': {
        # Navigation
        'home': 'Accueil',
        'campus_gallery': 'Galerie du campus',
        'my_likes': 'Mes favoris',
        'login': 'Connexion',
        'register': 'Inscription',
        'page_title_shop': 'Alumni Shop - School Badge Collection',
        'alumni_shop': 'Alumni Shop',
        'home_tab': 'Home',
        'share_wonderful_content': 'Share School Badge Collection',
        'please_wait': 'Please wait...',
        'from_schoolbadge': 'From School Badge Collection',
        'share_to': 'Share to',
        'wechat': 'WeChat',
        'weibo': 'Weibo',
        'douyin': 'Douyin',
        'xiaohongshu': 'Xiaohongshu',
        'generate_qr': 'Generate QR Code',
        'scan_qr_share': 'Scan to Share',
        'scan_qr_desc': 'Scan QR code to open link',
        'my_orders_tab': 'My Orders',
        'alumni_discount': 'Alumni Exclusive Discount',
        'shop_products': 'Selected badges and creative products, limited time!',
        'buy_now': 'Shop Now',
        'all_categories': 'All',
        'logout': 'Déconnexion',
        'language': 'Langue',
        'admin_panel': 'Admin',
        
        # Home
        'hero_title': 'Explorer les universités mondiales',
        'page_title_home': 'Accueil',
        'membership_unlock': 'Unlock more privileges and enjoy exclusive services',
        'open_vip_privilege': 'Upgrade for VIP privileges',
        'valid_until': 'Valid until',
        'select_membership_level': 'Select Membership Level',
        'membership_benefits': 'Membership Benefits',
        'unlimited_sharing': 'Unlimited Sharing',
        'remove_ads': 'Ad-Free',
        'exclusive_badge': 'Exclusive Badge',
        'hd_download': 'HD Download',
        'priority_activities': 'Priority Events',
        'exclusive_support': 'Exclusive Support',
        'member_orders': 'Membership Orders',
        'shop_orders': 'Shop Orders',
        'recommended': 'Recommended',
        'hero_subtitle': 'Découvrez des établissements prestigieux du monde entier',
        'search_placeholder': 'Rechercher par nom, ville ou pays...',
        'all_regions': 'Toutes les régions',
        'all_countries': 'Tous les pays',
        'all_cities': 'Toutes les villes',
        'all_districts': 'Tous les districts',
        'all_types': 'Tous les types',
        'university': 'Université',
        'middle_school': 'Collège',
        'elementary_school': 'École primaire',
        'kindergarten': 'Maternelle',
        'clear_filters': 'Effacer les filtres',
        'schools_count': '{} établissements trouvés',
        'no_schools': 'Aucun établissement trouvé',
        'view_all': 'Voir tous',
        
        # School Card
        'founded': 'Fondé',
        'principal': 'Directeur / Présidente',
        'address': 'Adresse',
        'location': 'Localisation',
        'region': 'Région',
        'district': 'District',
        'supervisor': 'Surveillant',
        'phone': 'Téléphone',
        'fax': 'Fax',
        'gender_type': 'Genre',
        'gender_coed': 'Mixte',
        'gender_girls': 'Filles',
        'gender_boys': 'Garçons',
        'finance_type': 'Type de financement',
        'finance_government': 'Public',
        'finance_aided': 'Subventionné',
        'finance_direct': 'APS',
        'finance_private': 'Privé',
        'session_type': 'Session',
        'session_wd': 'Journée complète',
        'session_am': 'Matin',
        'session_pm': 'Après-midi',
        'school_code': 'Code école',
        'school_type': "Type d'école",

        # School Detail
        'back': 'Retour à tous les établissements',
        'like': 'Favoris',
        'liked': 'Dans vos favoris',
        'download': 'Télécharger',
        'likes_count': '{} mentions J\'aime',
        'motto_label': 'Devise',
        'website': 'Site officiel',
        'visit_website': 'Voir le site',
        'visit_website_full': 'Visiter le site de {}',
        'view_on_maps': 'Voir sur Google Maps',
        'map_location': 'Emplacement sur la carte',
        'description_title': 'À propos',
        
        # Auth
        'username': 'Nom d\'utilisateur',
        'password': 'Mot de passe',
        'email': 'Email',
        'all_required': 'Tous les champs sont requis.',
        'register_success': 'Inscription réussie! Veuillez vous connecter.',
        'username_exists': 'Nom d\'utilisateur ou email déjà utilisé.',
        'login_error': 'Nom d\'utilisateur ou mot de passe incorrect.',
        'welcome_back': 'Bienvenue, {}!',
        'logout_success': 'Vous avez été déconnecté.',
        'please_login': 'Veuillez vous connecter pour accéder à cette page.',
        'already_account': 'Vous avez déjà un compte?',
        'no_account': 'Pas de compte?',
        
        # Admin
        'admin': 'Admin',
        'dashboard': 'Tableau de bord',
        'manage_schools': 'Gérer les écoles',
        'manage_users': 'Gérer les utilisateurs',
        'settings': 'Paramètres',
        'add_school': 'Ajouter une école',
        'edit_school': 'Modifier l\'école',
        'delete_school': 'Supprimer l\'école',
        'school_name': 'Nom de l\'école',
        'school_name_cn': 'Nom en chinois',
        'region': 'Région',
        'country': 'Pays',
        'city': 'Ville',
        'description': 'Description',
        'badge': 'Écusson',
        'motto': 'Devise',
        'founded_year': 'Année de fondation',
        'principal_name': 'Nom du directeur',
        'save': 'Enregistrer',
        'cancel': 'Annuler',
        'delete': 'Supprimer',
        'confirm_delete': 'Êtes-vous sûr?',
        'success': 'Succès!',
        'error': 'Erreur!',
        'no_permission': 'Vous n\'avez pas la permission.',
        'welcome_admin': 'Bienvenue, Admin!',
        'total_schools': 'Total écoles',
        'total_users': 'Total utilisateurs',
        'total_visits': 'Total visites',
        
        # Footer
        'copyright': '© 2024 Collection d\'écussons scolaires',
    },
    'de': {
        # Navigation
        'home': 'Startseite',
        'campus_gallery': 'Campus-Galerie',
        'my_likes': 'Meine Favoriten',
        'login': 'Anmelden',
        'register': 'Registrieren',
        'logout': 'Abmelden',
        'language': 'Sprache',
        'admin_panel': 'Admin',
        
        # Home
        'hero_title': 'Weltweite Universitäten erkunden',
        'page_title_home': 'Startseite',
        'hero_subtitle': 'Entdecken Sie prestigeträchtige Institutionen weltweit',
        'search_placeholder': 'Suchen nach Name, Stadt oder Land...',
        'all_regions': 'Alle Regionen',
        'all_countries': 'Alle Länder',
        'all_cities': 'Alle Städte',
        'all_districts': 'Alle Stadtteile',
        'all_types': 'Alle Typen',
        'university': 'Universität',
        'middle_school': 'Gymnasium',
        'elementary_school': 'Grundschule',
        'kindergarten': 'Kindergarten',
        'clear_filters': 'Filter löschen',
        'schools_count': '{} Schulen gefunden',
        'no_schools': 'Keine Schulen gefunden',
        'view_all': 'Alle anzeigen',
        
        # School Card
        'founded': 'Gegründet',
        'principal': 'Rektor / Direktorin',
        'address': 'Adresse',
        'location': 'Standort',
        'region': 'Region',
        'district': 'Stadtteil',
        'supervisor': 'Aufsichtsperson',
        'phone': 'Telefon',
        'fax': 'Fax',
        'gender_type': 'Geschlecht',
        'gender_coed': 'Gemischt',
        'gender_girls': 'Mädchen',
        'gender_boys': 'Jungen',
        'finance_type': 'Finanzierungsart',
        'finance_government': 'Staatlich',
        'finance_aided': 'Zuschuss',
        'finance_direct': 'Direkte Hilfe',
        'finance_private': 'Privat',
        'session_type': 'Sitzung',
        'session_wd': 'Ganzer Tag',
        'session_am': 'Vormittag',
        'session_pm': 'Nachmittag',
        'school_code': 'Schulcode',
        'school_type': 'Schultyp',

        # School Detail
        'back': 'Zurück zu allen Schulen',
        'like': 'Merken',
        'liked': 'Gemerkt',
        'download': 'Herunterladen',
        'likes_count': '{} Likes',
        'motto_label': 'Wahlspruch',
        'website': 'Offizielle Website',
        'visit_website': 'Website besuchen',
        'visit_website_full': '{} Website besuchen',
        'view_on_maps': 'Auf Google Maps ansehen',
        'map_location': 'Kartenstandort',
        'description_title': 'Über',
        
        # Auth
        'username': 'Benutzername',
        'password': 'Passwort',
        'email': 'E-Mail',
        'all_required': 'Alle Felder sind erforderlich.',
        'register_success': 'Registrierung erfolgreich! Bitte anmelden.',
        'username_exists': 'Benutzername oder E-Mail bereits vergeben.',
        'login_error': 'Ungültiger Benutzername oder Passwort.',
        'welcome_back': 'Willkommen zurück, {}!',
        'logout_success': 'Sie wurden abgemeldet.',
        'please_login': 'Bitte anmelden, um auf diese Seite zuzugreifen.',
        'already_account': 'Bereits registriert?',
        'no_account': 'Noch kein Konto?',
        
        # Admin
        'admin': 'Admin',
        'dashboard': 'Dashboard',
        'manage_schools': 'Schulen verwalten',
        'manage_users': 'Benutzer verwalten',
        'settings': 'Einstellungen',
        'add_school': 'Schule hinzufügen',
        'edit_school': 'Schule bearbeiten',
        'delete_school': 'Schule löschen',
        'school_name': 'Schulname',
        'school_name_cn': 'Chinesischer Name',
        'region': 'Region',
        'country': 'Land',
        'city': 'Stadt',
        'description': 'Beschreibung',
        'badge': 'Wappen',
        'motto': 'Wahlspruch',
        'founded_year': 'Gründungsjahr',
        'principal_name': 'Rektorname',
        'save': 'Speichern',
        'cancel': 'Abbrechen',
        'delete': 'Löschen',
        'confirm_delete': 'Sind Sie sicher?',
        'success': 'Erfolg!',
        'error': 'Fehler!',
        'no_permission': 'Sie haben keine Berechtigung.',
        'welcome_admin': 'Willkommen, Admin!',
        'total_schools': 'Gesamt Schulen',
        'total_users': 'Gesamt Benutzer',
        'total_visits': 'Gesamt Besuche',
        
        # Footer
        'copyright': '© 2024 Schulwappen-Sammlung',
    },
    'ja': {
        # Navigation
        'home': 'ホーム',
        'campus_gallery': 'キャンパスギャラリー',
        'my_likes': 'マイリスト',
        'login': 'ログイン',
        'register': '登録',
        'logout': 'ログアウト',
        'language': '言語',
        'admin_panel': '管理画面',
        
        # Home
        'hero_title': '世界の名門大学を探す',
        'page_title_home': 'ホーム',
        'hero_subtitle': '世界の名門校を探ろう',
        'search_placeholder': '学校名、都市、国で検索...',
        'all_regions': 'すべての地域',
        'all_countries': 'すべての国',
        'all_cities': 'すべての都市',
        'all_districts': 'すべての地区',
        'all_types': 'すべてのタイプ',
        'university': '大学',
        'middle_school': '中学校',
        'elementary_school': '小学校',
        'kindergarten': '幼稚園',
        'clear_filters': 'フィルターをクリア',
        'schools_count': '{} 校見つかりました',
        'no_schools': '条件に一致する学校がありません',
        'view_all': 'すべて見る',
        
        # School Card
        'founded': '創立',
        'principal': '学園長 / 校長',
        'address': '住所',
        'location': '所在地',
        'region': '地域',
        'district': '地区',
        'supervisor': '校監',
        'phone': '電話',
        'fax': 'Fax',
        'gender_type': '性别',
        'gender_coed': '共学',
        'gender_girls': '女子校',
        'gender_boys': '男子校',
        'finance_type': '办学类型',
        'finance_government': '官立',
        'finance_aided': '助成',
        'finance_direct': '直税',
        'finance_private': '私立',
        'session_type': '授業時間',
        'session_wd': '全日制',
        'session_am': '午前',
        'session_pm': '午後',
        'school_code': '学校番号',
        'school_type': '学校種別',

        # School Detail
        'back': '学校リストに戻る',
        'like': 'お気に入りに追加',
        'liked': 'お気に入り済み',
        'download': 'ダウンロード',
        'likes_count': '{} 件のいいね',
        'motto_label': '校訓',
        'website': '公式サイト',
        'visit_website': '公式サイトを見る',
        'visit_website_full': '{} 公式サイトを見る',
        'view_on_maps': 'Google マップで見る',
        'map_location': '地図位置',
        'description_title': '学校について',
        
        # Auth
        'username': 'ユーザー名',
        'password': 'パスワード',
        'email': 'メールアドレス',
        'all_required': 'すべての項目を入力してください。',
        'register_success': '登録完了！ログインしてください。',
        'username_exists': 'ユーザー名またはメールアドレスは既に使われています。',
        'login_error': 'ユーザー名またはパスワードが正しくありません。',
        'welcome_back': 'おかえりなさい、{}さん！',
        'logout_success': 'ログアウトしました。',
        'please_login': 'このサイトにアクセスするにはログインしてください。',
        'already_account': 'アカウントをお持ちですか？',
        'no_account': 'アカウントをお持ちでないですか？',
        
        # Admin
        'admin': '管理者',
        'dashboard': 'ダッシュボード',
        'manage_schools': '学校管理',
        'manage_users': 'ユーザー管理',
        'settings': '設定',
        'add_school': '学校を追加',
        'edit_school': '学校を編集',
        'delete_school': '学校を削除',
        'school_name': '学校名',
        'school_name_cn': '中国語名',
        'region': '地域',
        'country': '国',
        'city': '都市',
        'description': '説明',
        'badge': '校章',
        'motto': '校訓',
        'founded_year': '創立年',
        'principal_name': '校長名',
        'save': '保存',
        'cancel': 'キャンセル',
        'delete': '削除',
        'confirm_delete': '削除してよろしいですか？',
        'success': '成功！',
        'error': 'エラー！',
        'no_permission': 'このページにアクセスする権限がありません。',
        'welcome_admin': '管理者님、안녕하세요！',
        'total_schools': '学校総数',
        'total_users': 'ユーザー総数',
        'total_visits': 'アクセス数',
        
        # Footer
        'copyright': '© 2024 校章コレクション',
    },
    'zh-TW': {
        # Navigation
        'home': '首頁',
        'campus_gallery': '校園風光',
        'my_likes': '我的收藏',
        'login': '登入',
        'register': '註冊',
        'logout': '登出',
        'language': '語言',
        'admin_panel': '管理後台',
        
        # Home
        'hero_title': '探索全球名校',
        'page_title_home': '首頁',
        'hero_subtitle': 'Discover prestigious institutions worldwide',
        'search_placeholder': '搜尋學校名稱、城市或國家...',
        'all_regions': '所有地區',
        'all_countries': '所有國家',
        'all_cities': '所有城市',
        'all_districts': '所有區域',
        'all_types': '所有類型',
        'university': '大學',
        'middle_school': '中學',
        'elementary_school': '小學',
        'kindergarten': '幼兒園',
        'clear_filters': '清除篩選',
        'schools_count': '共 {} 所學校',
        'no_schools': '沒有找到符合條件的學校',
        'view_all': '查看全部學校',
        
        # School Card
        'founded': '建校年份',
        'principal': '現任校長',
        'address': '詳細地址',
        'location': '位置',
        'region': '地區',
        
        # School Detail
        'back': '返回學校列表',
        'like': '收藏',
        'liked': '已收藏',
        'download': '下載校徽',
        'likes_count': '{} 人收藏',
        'motto_label': '校訓',
        'website': '官方網站',
        'phone': '電話',
        'fax': '傳真',
        'principal_label': '校長',
        'supervisor': '校監',
        'school_type': '學校類別',
        'gender': '招生性別',
        'session': '授課時間',
        'finance_type': '辦學類型',
        'district': '所屬區域',
        'source': '數據來源',
        
        # Search Filters
        'search': '搜尋',
        
        # Social
        'posts': '貼文',
        'create_post': '發布貼文',
        'write_something': '寫下你的想法...',
        'like': '按讚',
        'comment': '評論',
        'share': '分享',
        'comments': '評論',
        'no_posts': '還沒有貼文',
        'write_first': '成為第一個發布貼文的人！',
        
        # Auth
        'username': '用戶名',
        'email': '電子郵箱',
        'password': '密碼',
        'confirm_password': '確認密碼',
        'forgot_password': '忘記密碼？',
        'no_account': '還沒有帳號？',
        'has_account': '已經有帳號？',
        'login_success': '登入成功',
        'logout_success': '已登出',
        'register_success': '註冊成功，請登入',
        'login_failed': '用戶名或密碼錯誤',
        'register_failed': '註冊失敗',
        
        # Admin
        'dashboard': '儀表盤',
        'users': '用戶管理',
        'schools': '學校管理',
        'logs': '操作日誌',
        'permissions': '權限管理',
        'add_school': '新增學校',
        'edit_school': '編輯學校',
        'delete_school': '刪除學校',
        'confirm_delete': '確認刪除？',
        'user_role': '用戶角色',
        'admin': '管理員',
        'normal_user': '普通用戶',
        'edit': '編輯',
        'delete': '刪除',
        'save': '儲存',
        'cancel': '取消',
        
        # Deep Search
        'deep_search': 'AI 深度搜尋',
        
        # Errors
        'error_404': '頁面不存在',
        'error_500': '伺服器錯誤',
        'no_permission': '您沒有權限訪問此頁面。',
        'welcome_admin': '管理員，您好！',
        'total_schools': '學校總數',
        'total_users': '用戶總數',
        'total_visits': '訪問量',
        
        # Footer
        'copyright': '© 2024 校徽網 · School Badge Collection',
    }
}

# Language names for selector
LANGUAGE_NAMES = {
    'zh': '中文简体',
    'zh-TW': '中文繁體',
    'en': 'English',
    'fr': 'Français',
    'de': 'Deutsch',
    'ja': '日本語'
}

def get_locale():
    """Get current locale from session or request."""
    if 'lang' in session:
        return session['lang']
    
    # Check Accept-Language header
    accept_language = request.headers.get('Accept-Language', '')
    for lang in accept_language.split(','):
        lang_code = lang.split(';')[0].strip().lower()
        if lang_code.startswith('zh'):
            if 'tw' in lang_code or 'hk' in lang_code:
                return 'zh-TW'
            return 'zh'
        elif lang_code.startswith('en'):
            return 'en'
        elif lang_code.startswith('fr'):
            return 'fr'
        elif lang_code.startswith('de'):
            return 'de'
        elif lang_code.startswith('ja'):
            return 'ja'
    
    return 'zh'  # Default to Chinese

def _(key):
    """Translate a key to the current language."""
    locale = get_locale()
    translations = TRANSLATIONS.get(locale)
    
    # 如果没有该语言的翻译，回退到简体中文
    if not translations:
        translations = TRANSLATIONS.get('zh', {})
    
    return translations.get(key, TRANSLATIONS['zh'].get(key, key))
