...
LOG_FILE = '/tmp/translate_conservative.log'
...
def call_minimax_translate(texts, max_retries=5):
    """带递增等待的翻译"""
...
Respond with ONLY the JSON array."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 失败，递增等待
            wait = (attempt + 1) * 30  # 30, 60, 90, 120, 150 秒
            log(f"    等待 {wait} 秒后重试 ({attempt+1}/{max_retries})...")
            time.sleep(wait)
            
        except Exception as e:
            wait = (attempt + 1) * 30
            log(f"    错误: {str(e)[:50]}, 等待 {wait} 秒...")
            time.sleep(wait)
...
def main():
    ...
    LOG_FILE = f'/tmp/translate_conservative_{os.getpid()}.log'
    if os.path.exists(LOG_FILE):
        os.remove(LOG_FILE)
    
    log("=" * 60)
    log("AI 翻译 - name_cn (超保守版)")
    log("策略: 每批30秒间隔, 每5批休息60秒")
    log("=" * 60)
    ...
    # 失败重试队列
    failed_batches = []
    batch_interval = 30  # 每批间隔 30 秒
    rest_interval = 60   # 每 5 批休息 60 秒
    ...
    for i in range(0, total_missing, batch_size):
        ...
        translations = call_minimax_translate(texts, max_retries=5)
    ...
    if failed_batches:
        log(f"\n{'=' * 60}")
        log(f"开始重试 {len(failed_batches)} 个失败批次...")
        log(f"重试间隔: 60 秒")
        log(f"{'=' * 60}")
        ...
        for batch_num, batch in failed_batches:
            ...
            translations = call_minimax_translate(texts, max_retries=3)
